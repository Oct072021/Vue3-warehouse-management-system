  ...
  <div class="tab-container">
      ...
      >
        <template #table>
          <el-table :data="tableData" border fit highlight-current-row style="width: 100%" draggable>
            ...
            <el-table-column class-name="status-col" :label="t(`orders.supplier`)" prop="supplier" min-width="110" />
    ...
    <el-dialog :title="titleMap[dialogStatus]" v-model="dialogFormVisible" align-center width="900px">
      <el-form ref="dataForm" :model="temp" label-position="left">
        ...
        <el-row :gutter="20">
          <el-col :span="8">
            <el-form-item :label="t(`orders.supplier`) + ':'" prop="supplier">
              {{ temp.supplier }}
            </el-form-item>
          </el-col>
        ...
        >
          ...
          <el-table-column class-name="status-col" :label="t(`orders.total`)" prop="supplier" min-width="90">
            <template #default="{ row }">
      ...
      <template #footer>
        <div class="dialog-footer">
          <template v-if="dialogStatus === 'audit'">
            <el-button type="success" @click="audit(temp, 1)">
              {{ t(`button.pass`) }}
            </el-button>
            <el-button type="danger" @click="audit(temp, 2)">
              {{ t(`button.noPass`) }}
            </el-button>
          </template>
...
import { auditOrder, allData, detailOrder, getList, removeOrder } from './service'
...
const handleDownload = throttle(async function () {
  ...
  import('@/vendor/Export2Excel').then((excel) => {
    ...
    excel.export_json_to_excel({
      header: tHeader,
      data,
      filename: `inbound-list-${searchQuery.area}`,
    })
